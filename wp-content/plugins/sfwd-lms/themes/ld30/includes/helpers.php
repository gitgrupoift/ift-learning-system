...
function learndash_lesson_row_class( $lesson = null, $has_access = false ) {
	...
	if ( ( isset( $is_current_lesson ) && $is_current_lesson ) || ( isset( $_GET['widget_instance']['widget_instance']['current_lesson_id'] ) && $_GET['widget_instance']['widget_instance']['current_lesson_id'] === $lesson['post']->ID ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Data is only used for conditional and not further processed.
...
add_filter(
	...
	function( $access, $post_id, $user_id ) {
		if ( ( is_single() ) && ( ! is_admin() ) ) {
			...
			if ( ( true === $access ) && ( ! empty( $lesson_id ) ) && ( learndash_is_sample( $lesson_id ) ) ) {
