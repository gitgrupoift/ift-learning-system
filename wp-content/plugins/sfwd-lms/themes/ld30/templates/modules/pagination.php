...
if ( ( isset( $pager_results ) ) && ( ! empty( $pager_results ) ) ) {
	...
	if ( $pager_results['total_pages'] > 1 ) {
		...
		?>
			...
			<?php } ?> data-context="<?php echo esc_attr( $pager_context ); ?>" data-paged="<?php echo esc_attr( $href_val_prefix . $next_page_number . $search_arg ); ?>" data-course_id="<?php echo esc_attr( $course_id ); ?>" <?php echo esc_attr( $data_lesson_id ); ?> class="<?php echo esc_attr( $pager_right_class ); ?>" <?php echo esc_attr( $pager_right_disabled ); ?> title="<?php esc_attr_e( 'Next Page', 'learndash' ); ?>">
			<?php if ( is_rtl() ) { ?>
				<span class="ld-icon-arrow-left ld-icon"></span></a>
			<?php } else { ?>
				<span class="ld-icon-arrow-right ld-icon"></span></a>
