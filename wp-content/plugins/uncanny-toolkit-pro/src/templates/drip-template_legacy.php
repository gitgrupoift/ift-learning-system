...
<?php if ( $has_course_content ) : ?>
	...
	if ( $show_course_content ) :
		...
		<div id="learndash_course_content" class="learndash_course_content">
			<!--<h4 id="learndash_course_content_title"><?php /*printf( _x( '%s Content', 'Course Content Label', 'learndash' ), LearnDash_Custom_Label::get_label( 'course' ) ); */
			?></h4>-->
					...
					<div id="lessons_list" class="lessons_list">
						...
						<?php foreach ( $lessons as $lesson ) :
							...
							if ( ! empty( $access_from ) && 'Available' !== $access_from ) {
								//$access_from                  = \uncanny_pro_toolkit\UncannyDripLessonsByGroup::adjust_for_timezone_difference( $access_from );
								$lesson["lesson_access_from"] = $access_from;

								$date_format      = get_option( 'date_format' );
								$time_format      = get_option( 'time_format' );
								$date             = \uncanny_pro_toolkit\UncannyDripLessonsByGroup::adjust_for_timezone_difference( $lesson['lesson_access_from'] );
							...
							<div class='post-<?php echo esc_attr( $lesson['post']->ID ); ?> <?php echo esc_attr( $lesson['sample'] ); ?>'>
								...
								<h4>
									<a class='<?php echo $is_not_available ? 'notavailable' : esc_attr( $lesson['status'] ); ?>' href='<?php echo esc_attr( $lesson['permalink'] ); ?>'><?php echo $lesson['post']->post_title; ?></a>
									...
									<?php */ ?>
										...
										SFWD_LMS::get_template(
											...
											array(
												...
												'lesson_access_from_int'  => strtotime( $date ),
												'lesson_access_from_date' => $date,
												'context'                 => 'course'
											), true
										);
										?>
